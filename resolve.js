...
exports.resolve = function(source, file, settings) {
  ...
  let configPath = get(settings, 'config');
  ...
  const resolveSync = getResolveSync(configPath, webpackConfig);
...
function getResolveSync(configPath, webpackConfig) {
  ...
  let cached = find(_cache, function(entry) {
    return isEqual(entry.key, cacheKey);
  });
  if (!cached) {
    cached = {
      key: cacheKey,
...
function createWebpack2ResolveSync(webpackRequire, resolveConfig) {
  ...
  return EnhancedResolve.create.sync(
    Object.assign({}, webpack2DefaultResolveConfig, resolveConfig)
  );
}
...
function createWebpack1ResolveSync(webpackRequire, resolveConfig, plugins) {
  ...
  const SyncNodeJsInputFileSystem = webpackRequire(
  ...
  const ModulesInDirectoriesPlugin = webpackRequire(
  ...
  const DirectoryDescriptionFilePlugin = webpackRequire(
    'enhanced-resolve/lib/DirectoryDescriptionFilePlugin'
  );
  const DirectoryDefaultFilePlugin = webpackRequire(
    'enhanced-resolve/lib/DirectoryDefaultFilePlugin'
  );
  ...
  const DirectoryDescriptionFileFieldAliasPlugin = webpackRequire(
    'enhanced-resolve/lib/DirectoryDescriptionFileFieldAliasPlugin'
  );
...
function findExternal(source, externals, context) {
  ...
  if (externals instanceof Array) {
    return externals.some(function(e) {
      return findExternal(source, e, context);
    });
  }
 ...
 * @type {Array}
 */
var webpack1DefaultMains = ['webpack', 'browser', 'web', 'browserify', ['jam', 'main'], 'main'];
...
function findConfigPath(configPath, packageDir) {
  ...
  if (configPath) {
    ...
    extensions.forEach(function(maybeExtension) {
      ...
      const maybePath = path.resolve(path.join(packageDir, `webpack.config${maybeExtension}`));
